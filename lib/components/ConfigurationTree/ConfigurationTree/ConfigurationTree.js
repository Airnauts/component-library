...
function appendBreadcrumbs(nodes, breadcrumbs) {
  ...
  if (node) {
    if (node.type === 'Area') {
      breadcrumbs.push({ name: node.name, active: node.state === 'clicked', id: node.id });
    }
  ...
  return (
    <React.Fragment>
      <div className="ct-breadcrumbs__container">
        {
          breadcrumbs.map(({ name, active, id }, index) => (
            <button
              className={`ct-breadcrumbs__element${active ? ' ct-breadcrumbs__element--active' : ''}`}
              onClick={() => {
                window.scrollTo({ left: 0, top: index === 0 ? 0 : (index * 200) + 110, behavior: 'smooth' });
                onNodeClick(id);
              }}
