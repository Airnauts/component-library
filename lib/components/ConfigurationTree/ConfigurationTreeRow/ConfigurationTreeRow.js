...
class ConfigurationTreeRow extends React.Component {
  ...
  getRowOffset() {
    ...
    if (pivotOffset && nodes.length < columnCount) {
      ...
      const additionalOffset = nodes.length === 1
        ? 0
        : columnWidth;
      const maxRowOffset =
        // eslint-disable-next-line no-mixed-operators
        (columnWidth * columnCount) - (visibleWidth + additionalOffset);
  ...
  getActiveNode() {
    ...
    const activeNode = nodes
      .map((node, index) => {
        ...
        const isLastVisible = visibleIndex === columnCount - 1;
        return {
          node,
          index,
          visibleIndex,
    ...
    return (
      <React.Fragment>
        <ConfigurationTreeRow
          ...
          parentLastVisible={activeNode.isLastVisible}
  ...
  renderNode(node, index) {
    const { nodes, parentNode, parentLastVisible } = this.props;
    const { rowOffset } = this.getRowOffset();
    ...
    if (node.type !== 'Add') {
      ...
      if (index === normalNodes.length - 1) {
        if (parentLastVisible) {
          // special case for active node being last visible node on the right
          lines.rightLine.show = nodes.length !== normalNodes.length;
        } else {
    ...
    } else if (normalNodes.length > 0 && parentLastVisible) {
