...
class SliderComp extends Component {
  ...
  onBarMouseUp() {
    ...
    if (isBarDragged) {
      ...
      const markersCount = Math.ceil((maxVal - minVal) / markerStep) + 1;
      const pxPerMarker = this.scrollBarMax / markersCount;
      const value = (Math.round(dragValue / pxPerMarker) * markerStep) + minVal;
      onChange({ name, value });
    }
  }
  ...
  valueToPct(value) {
    ...
    return ((value - minVal) / maxVal) * 100;
  }
  ...
  render() {
    ...
    return (
      <div className="SliderComp">
        ...
        <div
          style={{ maxWidth: `calc(${trackWidth}% + 4px)`, marginLeft: `calc((${markerWidth / 2}%) - 2px)` }}
